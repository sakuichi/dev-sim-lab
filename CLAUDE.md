...
ユーザー提案・調査済み(2026-07-13、反証確認込み。未採用):
- 仕様は喫煙所で決まる — 語源級の一致: "smoke-filled room"(1920年ハーディング指名、シカゴ・ブラックストーンホテルの密室政治の故事)。背骨は手続き的公正のボイス効果(Lind et al. 1990ほか: 発言機会があると不利な結果でも受け入れやすい)。反証・両論: 非公式な調整(根回し等)自体には合意形成上の価値があるという系譜もある。ページ化するなら主語は「非公式さ」ではなく「関わるべき人の不在のまま決まること」にする
- 学習性無力感 — 古典はSeligman & Maier 1967だが、**提唱者本人が2016年の"Learned Helplessness at Fifty"で理論を半分撤回**: 無力感は学習されるのではなく長引く制御不能ストレスへの生得的デフォルト反応で、学習されるのは「制御できる感覚」の方。ページはこの2016年改訂を軸に「制御感を学習できる環境か」という組織主語で書く。メンタルヘルス隣接のためセンシティブ規約を全面適用(診断ツールではない免責必須)
- 仕事ができる人に仕事が集まる — Collaborative Overload(Cross/Rebele/Grant, HBR 2016): 300以上の組織で、付加価値の高い協働の20〜35%が3〜5%の社員から生まれ、その人たちがボトルネック化し燃え尽きる。バス係数(No.29)と相互参照。両論: 有能者への難タスク集中は短期的には合理的(比較優位)であり、トレードオフとして描く。近接概念: パレートの法則(80/20)は「集中している」という分布の記述で、本テーマはその集中が生まれ、拡大し、コストを生む動的メカニズムの話——分布(静的)とループ(動的)の関係
