...
**残り5テーマは選定済みで、調査・設計も完了している。実装仕様は `docs/theme-designs.md` を正とすること**(定量化の罠→タイヤのブランコ→バス係数→90-90ルール→コンウェイの法則、の順)。以下のバックログは31本達成後の拡張候補として残す。

## 既知のバックログ(31本達成後の拡張候補。採用はユーザーと相談して決める。勝手に選ばない)
...
- 信頼ポイント(ユーザーの造語) — **採用確定(2026-07-13)、切り口も合意済み**: 「地位⇄発言時間⇄影響力」の双方向強化ループをモデル化する。背骨は地位特性理論/期待状態理論(Berger et al., 数十年の実験的再現あり。発言機会の配分自体が地位で決まることを含む)。Hollanderの特異性信用(1958)は「信用の銀行口座」メタファーとして限界(小集団実験中心・職場への外挿は拡張)込みで紹介。バブル仮説(MacLaren et al. 2020, 発言時間がリーダー認知を予測、内生性対処を試みた研究だが因果方向は議論あり)は補助線。単方向の主張を断定しない。センシティブ配慮: 記述/規範の分離を冒頭免責で宣言、主語は会議運営・評価プロセス、損失は双方向(低信用者の良案が埋もれる+高信用者の悪案が素通り)で提示、個人向け処方(新人は我慢して信用を貯めよ等)は書かない
